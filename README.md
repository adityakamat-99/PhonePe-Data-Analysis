...
## 🛠️ Tech Stack
* **Python:** Pandas, NumPy, Matplotlib Data Imputation, Cleaning, Exploratory Data Analysis (`Notebooks/Phone Pe Problems.ipynb`)
* **SQL:** MySQL: Aggregations, Window Functions, CTEs, View Creation (`SQL Scripts/SQL problems.sql`)
* **Data Visualization:** Microsoft Power BI (`Dashboard/Phone Pe Dashboard.pbix`)
* **Raw Data:** (`Data/phonepe_raw_data.xlsx`)
...
5. **App Engagement & Growth:** Total transactions and app opens experienced exponential growth from 2018 to 2021[cite: 6], which correlates directly with the sharp, sustained rise in peer-to-peer and merchant transfer amounts over the same period.

## 📂 Repository Contents
* **`Data/phonepe_raw_data.xlsx`**: The foundational dataset containing state and district-level transaction, user, and demographic data.
* **`Notebooks/Phone Pe Problems.ipynb`**: Python code detailing the data wrangling process, including the imputation of missing geographical demographics.
* **`SQL Scripts/SQL problems.sql`**: Advanced SQL queries utilizing CTEs and window functions to solve specific business problems.
* **`SQL Scripts/SQL exports.sql`**: Structured SQL views created to feed clean, aggregated data directly into the Power BI dashboard.
* **`Dashboard/Phone Pe Dashboard.pbix`**: The final interactive Power BI dashboard file.
